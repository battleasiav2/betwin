...
@section('panel')
    <div class="row">
        <div class="col-12">
            <div class="row gy-4">
                ...
                <div class="col-xxl-3 col-sm-6">
                    <x-widget
                              ...
                              icon="las la-sync"
                              value="{{ showAmount($user->turnover_requirement) }}"
                              bg="1"
                              type="2" />
                </div>
            ...
            <div class="d-flex flex-wrap gap-3 mt-4">
                ...
                {{-- New Turnover Control Buttons --}}
                <div class="flex-fill">
                    <button data-bs-toggle="modal" data-bs-target="#turnoverModal" class="btn btn--info btn--shadow w-100 btn-lg turnover-btn" data-act="add">
                        <i class="las la-plus-circle"></i> @lang('Turnover')
                    </button>
                </div>

                <div class="flex-fill">
                    <button data-bs-toggle="modal" data-bs-target="#turnoverModal" class="btn btn--warning btn--shadow w-100 btn-lg turnover-btn" data-act="sub">
                        <i class="las la-minus-circle"></i> @lang('Turnover')
                    </button>
                </div>
            </div>
    ...
    {{-- Turnover Requirement MODAL --}}
    <div id="turnoverModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><span class="t-type"></span> <span>@lang('Turnover Requirement')</span></h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                {{-- নোট: এখানে রাউটটি আমরা পরের ধাপে ডিফাইন করব --}}
                <form action="{{ route('admin.users.add.sub.turnover', $user->id) }}" method="POST">
                    @csrf
                    <input type="hidden" name="act">
                    <div class="modal-body">
                        <div class="form-group">
...
@push('script')
    <script>
        (function($) {
            ...
            // Turnover Modal Script
            $('.turnover-btn').on('click', function() {
                var act = $(this).data('act');
                $('#turnoverModal').find('input[name=act]').val(act);
                if (act == 'add') {
                    $('.t-type').text('Add');
                } else {
                    $('.t-type').text('Subtract');
                }
            });
