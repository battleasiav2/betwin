...
$sportsGames = [
    ...
    [
        ...
        'name' => 'DP Sports',
        'provider' => 'dpsports',
        'img' => asset('assets/images/sports/dpsports.svg'),
    ],
    [
        'id' => 'e130116fdc9bcde2dbb31735b6c365d6',
        'name' => 'DP Esports',
        'provider' => 'dpesports',
        'img' => asset('assets/images/sports/dpesports.svg'),
    ],
];
...
@foreach ($sportsGames as $game)
    ...
    @endphp
    <div class="swiper-slide game-item-box game-card" data-category="sports" data-provider="{{ $prov }}" data-game-id="{{ $game['id'] }}" data-game-name="{{ $game['name'] }}" data-game-img="{{ $img }}">
        @auth
            <a href="{{ url('user/jili/launch?game_code='.$game['id'].'&provider='.$prov) }}" class="game-card-img" title="{{ $game['name'] }}">
        @else
