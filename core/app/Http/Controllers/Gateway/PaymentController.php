...
class PaymentController extends Controller {
    ...
    public function depositInsert(Request $request) {
        ...
        // বোনাস চেক: প্রমোশন সিলেক্ট করলে মিনিমাম লিমিট চেক করা
        if ($request->promotion_id) {
            $promotion = Promotion::find($request->promotion_id);
            if ($promotion && $request->amount < $promotion->min_limit) {
                $notify[] = ['error', 'Minimum ' . showAmount($promotion->min_limit) . ' required for this promotion'];
                return back()->withNotify($notify);
            }
        }
    ...
    public static function userDataUpdate($deposit, $isManual = null) {
        if ($deposit->status == Status::PAYMENT_INITIATE || $deposit->status == Status::PAYMENT_PENDING) {
            ...
            if ($deposit->promotion_id) {
                $promotion = Promotion::find($deposit->promotion_id);
                if ($promotion && $promotion->status == 1 && $deposit->amount >= $promotion->min_limit) {
                    
                    $bonusAmount = ($deposit->amount * $promotion->bonus_percent) / 100;
                    ...
                    $bonusTurnover = $bonusAmount * $promotion->turnover_multiplier;
                    $user->turnover_requirement += $bonusTurnover;
                    ...
                    $bonusTrx->charge = 0;
                    $bonusTrx->trx_type = '+';
                    $bonusTrx->details = 'Received Bonus for ' . $promotion->title;
                    $bonusTrx->trx = $deposit->trx;
                    $bonusTrx->remark = 'promotion_bonus';
                    $bonusTrx->save();
                }
            }
