...
class ManageUsersController extends Controller {
    ...
    /**
     * Update User Turnover Requirement Manually by Admin
     */
    public function addSubTurnover(Request $request, $id)
    {
        $request->validate([
            'amount' => 'required|numeric|gt:0',
            'act'    => 'required|in:add,sub',
        ...
        if ($request->act == 'add') {
            $user->turnover_requirement += $amount;
            $notify[] = ['success', showAmount($amount) . ' turnover added successfully'];
        } else {
            if ($amount > $user->turnover_requirement) {
                $user->turnover_requirement = 0;
            } else {
                $user->turnover_requirement -= $amount;
            }
            $notify[] = ['success', showAmount($amount) . ' turnover subtracted successfully'];
        }
