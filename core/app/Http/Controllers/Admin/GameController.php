...
class GameController extends Controller
{
    ...
    {
        $pageTitle = "Game Logs";
        $logs      = GameLog::where('status', Status::ENABLE)->searchable(['user:username', 'game_name'])->filter(['win_status'])->with('user', 'game')->latest('id');
        
        $widget['total_win_amount']  = (clone $logs)->where('win_status', '!=', 0)->sum('win_amo');
        $widget['total_loss_amount'] = (clone $logs)->where('win_status', 0)->sum('invest');

        $logs = $logs->paginate(getPaginate());
        return view('admin.game.log', compact('pageTitle', 'logs', 'widget'));
    }
